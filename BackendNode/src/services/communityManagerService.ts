...
export default class CommunityManagerService implements ICommunityManagerService {
  constructor(
    ...
    public async createCommunityManager(communityManagerDTO: ICommunityManagerDTO): Promise<Result<ICommunityManagerDTO>> {
        try {
            ...
            */
            ...
            communityManagerOrError = CommunityManagerMap.toDomainFromDto(communityManagerDTO.id,communityOrError.getValue(), userOrError);
    ...
    public async updateCommunityManager(communityManagerDTO: ICommunityManagerDTO): Promise<Result<ICommunityManagerDTO>> {
        try {
            ...
            if (communityManagerOrError.getValue().community.id.toString() !== communityManagerDTO.communityId && communityManagerDTO.communityId !== undefined) {
                const communityOrError = await this.communityRepo.findById(communityManagerDTO.communityId);
                if (communityOrError.isFailure) {
                    return Result.fail<ICommunityManagerDTO>("Battery doesn't exist");
                }
                communityManagerOrError.getValue().community = communityOrError.getValue();
            }
            if (communityManagerOrError.getValue().user.id.toString() !== communityManagerDTO.userId && communityManagerDTO.userId !== undefined) {
                 const userOrError = await this.userRepo.findById(communityManagerDTO.userId);
/*                if (userOrError.isFailure) {
                    return Result.fail<ICommunityManagerDTO>("User doesn't exist");
                } */
