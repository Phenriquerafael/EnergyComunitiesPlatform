...
export default class ProsumerService implements IProsumerService {
  ...
  public async createProsumer(prosumerDTO: IProsumerDTO): Promise<Result<IProsumerDTO>> {
    try {
      ...
      prosumerOrError = ProsumerMap.toDomainFromDto(
        prosumerDTO.id,
        batteryOrError.getValue(),
        userOrError,
  ...
  public async updateProsumer(prosumerDTO: IProsumerDTO): Promise<Result<IProsumerDTO>> {
    try {
      ...
      if (
        prosumerOrError.getValue().battery.id.toString() !== prosumerDTO.batteryId &&
        prosumerDTO.batteryId !== undefined
      ) {
        const batteryOrError = await this.batteryRepo.findById(prosumerDTO.batteryId);
        if (batteryOrError.isFailure) {
  ...
  public async findProsumersWithoutCommunity(): Promise<Result<IProsumerDataDTO[]>> {
    try {
      const prosumersOrError = await this.prosumerRepo.findProsumersWithoutCommunity();
      if (prosumersOrError.isFailure) {
        return Result.fail<IProsumerDataDTO[]>('Error finding prosumers without community');
      }
      ...
      console.log('Error finding prosumers without community: ', error);
      return Result.fail<IProsumerDataDTO[]>('Error finding prosumers without community');
    }
  }
        ...
        prosumers.map(async (prosumer) => {
          ...
          if (saveResult.isFailure) {
            return Result.fail<void>(
              `Error saving prosumer with ID ${prosumer.prosumerId} after removing from community`,
            );
          }
        }),
      );
