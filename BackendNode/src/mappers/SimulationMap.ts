...
export class SimulationMap {
  public static toDto(simulation: Simulation): ISimulationDTO {
    return {
      ...
      activeAttributes:
        simulation.props.activeAttributes?.map((attr) => ({
  ...
  public static toDto2(simulation: Simulation): ISimulationDTO2 {
    return {
      id: simulation.id.toString(),
      startDate: simulation.props.startDate,
      endDate: simulation.props.endDate,
      description: simulation.props.description || '',
      communityId: simulation.props.community.id.toString(),
      communityCountry: simulation.props.community.props.country || '',
      communityCode: simulation.props.community.props.countryCode || '',
    } as ISimulationDTO2;
  }

  public static async toDomain(
    rawSimulation: ISimulationPersistence & { community: PrismaCommunity },
  ): Promise<Simulation> {
    ...
    const simulationOrError = Simulation.create(
      {
        ...
        activeAttributes:
          rawSimulation.activeAttributes?.map((attr) =>
    ...
    const simulationOrError = Simulation.create(
      {
        ...
        community: community,
        activeAttributes:
          simulationDTO.activeAttributes?.map((attr) =>
            ActiveAttributes.create({
              prosumerId: attr.prosumerId,
              profileLoad: attr.profileLoad,
              stateOfCharge: attr.stateOfCharge,
